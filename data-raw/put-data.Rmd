...
```{r file_colnames}
file_colnames <- function(x) {
  if (!file.exists(x)) {
    x <- sub("_AUS_([A-Z]+)\\.csv", "_AUST_\\1.csv", x)
  }
  temp <- data.table::fread(x, nrows = 1)
  names(temp)
}
```

```{r fread}
# for file existence and different column names
fread <- function(file, select = NULL, ...) {
  x <- file
  if (!file.exists(x)) {
    x <- sub("_AUS_([A-Z0-9]+)\\.csv", "_AUST_\\1.csv", x)
  }
  if (!file.exists(x)) {
    x <- sub("/AUST/", "/AUS/", x)
  }
  # handle differently cased colnames
  if (!is.null(select)) {
    x.colnames <- file_colnames(x)
    absent_colnames <- setdiff(select, x.colnames)
    select <- x.colnames[tolower(x.colnames) %in% tolower(select)]
  }
  
  data.table::fread(x, select = select, ...)
}
```

```{r}
Metadata <-
  read_excel("data-raw/data-packs/Metadata/Metadata_2021_GCP_DataPack_R1.xlsx",
             sheet = 2,
             skip = 10) %>%
  as.data.table
```
...
```{r SA21_decoder}
SA21_decoder <- fread("data-raw/SA21_decoder.csv")
```

```{r Region_key}
Region_key <- 
  fread(paste0('./data-raw/data-packs/data/', Region, '/AUST/2021Census_G01_AUST_', Region, '.csv')) %>%
  names %>%
  extract2(1)
```
...
freadG <- function(g, ...) {
  ...
  file_list <-
    list.files(path = file.path('./data-raw/data-packs/data/', Region),
               # Files may be of the form ..G08_AUS.. or ..G09B_AUS..
               pattern = paste0('2021Census_G', g0, '[A-Z]?_AUST_', Region, '\\.csv$'),
               recursive = TRUE,
               full.names = TRUE)
...
```{r total_adults}
total_adults <- 
  fread("./data-raw/data-packs/data/AUST/2021Census_G01_AUS_AUS.csv") %>%
...
decode_country <- function(out) {
  ...
  out[CountryOfBirth == "North Macedonia", CountryOfBirth := "MKD"]
...
Mop <- function(DT, value.name = "persons", suborder = NULL) {
  ...
  if (Region %notin% c("SA1", "POA")) {
    decoder <- 
      switch(Region,
             "SA2" = SA21_decoder[ASGS_Structure == Region,
                                  .(SA2_MAIN21 = Census_Code_2021,
                                    SA2_NAME21 = Census_Name_2021)],
             "SA3" = SA21_decoder[ASGS_Structure == Region,
                                  .(SA3_MAIN21 = Census_Code_2021,
                                    SA3_NAME21 = Census_Name_2021)],
             "SA4" = SA21_decoder[ASGS_Structure == Region,
                                  .(SA4_MAIN21 = Census_Code_2021,
                                    SA4_NAME21 = Census_Name_2021)],
             "STE" = SA21_decoder[ASGS_Structure == Region,
                                  .(STE_MAIN21 = Census_Code_2021,
                                    STE_NAME21 = Census_Name_2021)],
             "LGA" = {
               NonABS_decoder[ASGS_Structure == Region,
                              .(Census_Code_2021, LGA_NAME21 = Census_Name_2021)]
             },
             "CED" = {
               NonABS_decoder[ASGS_Structure == Region,
                              .(Census_Code_2021, CED_NAME21 = Census_Name_2021)]
             },
             "SED" = {
               NonABS_decoder[ASGS_Structure == Region,
                              .(Census_Code_2021, SED_NAME21 = Census_Name_2021)]
             },
             "SSC" = {
               NonABS_decoder[ASGS_Structure == Region,
                              .(Census_Code_2021, SSC_NAME21 = Census_Name_2021)]
             },
             "GCCSA" = {
               read_excel("./data-raw/data-packs/Metadata/2021Census_geog_desc_1st_release.xlsx",
                          sheet = "2021_ASGS_GCCSA_Structure") %>%
                 as.data.table %>%
                 .[, .(Census_Code_2021, GCCSA_NAME21 = Census_Name_2021)]
             },
             NULL) %>%
      unique %>%
  ...
  if (get0("in_for_loop", ifnotfound = FALSE)) cat("=")
...
ISO3166 <-
  if (file.exists("data-raw/ISO-3166-countries.csv")) {
    ...
    fread(iso3166_url, encoding = "UTF-8") %T>%
...
CED_decoder <-
  ...
  setDT %>%
  .[ASGS_Structure == "CED"] %>%
  setnames("Census_Code_2021", Region_key) %>%
  setnames("Census_Name_2021", "Electoral_division") %>%
...
```{r Region__Age}
fread(paste0('./data-raw/data-packs/data/', Region, '/AUST/2021Census_G01_AUS_', Region, '.csv')) %>%
...
fread(paste0('./data-raw/data-packs/data/', Region, '/AUST/2021Census_G01_AUS_', Region, '.csv'), 
  ...
  melt.data.table(id.vars = Region_key,
                  value.name = "persons") %>%
  .[variable == "Lang_used_home_Eng_only_P", OnlyEnglishSpokenHome := TRUE] %>%
  .[variable == "Lang_used_home_Oth_Lang_P", OnlyEnglishSpokenHome := FALSE] %>%
  Mop
```
...
fread(paste0('./data-raw/data-packs/data/', Region, '/AUST/2021Census_G01_AUS_', Region, '.csv'), 
        select = c(Region_key, 
                   "Lang_used_home_Eng_only_F",
                   "Lang_used_home_Eng_only_M",
                   "Lang_used_home_Oth_Lang_F",
                   "Lang_used_home_Oth_Lang_M")) %>%
  melt.data.table(id.vars = Region_key, 
                  value.name = "persons") %>%
  .[variable %pin% "Lang_used_home_Eng_only", OnlyEnglishSpokenHome := TRUE] %>%
  .[variable %pin% "Lang_used_home_Oth_Lang", OnlyEnglishSpokenHome := FALSE] %>%
  .[variable %pin% "F$", Sex := "F"] %>%
  .[variable %pin% "M$", Sex := "M"] %>%
  Mop
```
...
fread(paste0('./data-raw/data-packs/data/', Region, '/AUST/2021Census_G02_AUS_', Region, '.csv')) %>%
...
freadG(8) %>%
  .[, .SD, .SDcols = c(Region_key,
                       grep("Tot_Resp", names(.), value = TRUE, ignore.case = TRUE))] %>%
  melt.data.table(id.vars = Region_key,
                  value.name = "responses") %>%
...
freadG(11, na.strings = "..") %>%
  ...
  .[grepl("UOLSE", variable), SpeaksEnglishOnly := FALSE] %>%
  ...
  .[, EnglishProficiency := factor(EnglishProficiency, 
                                   levels = Census2016.spec::MEASURE_VARS$EnglishProficiency$permitted_values,
                                   ordered = TRUE)] %>%
  .[] %>%
...
list.files(path = paste0('./data-raw/data-packs/data/', Region, '/AUST/'),
             pattern = paste0('2021Census_G13[A-Z]_AUS_', Region, '\\.csv$'),
             full.names = TRUE) %>%
...
freadG(15) %>%
  .[, .SD, .SDcols = names(.) %pin% c(Region_key, 
                                      "^([A-Za-z_]+)_Tot_\\1_[MF]$", "Preschool_[MF]",
                                      "Tert_(Voc|Uni).*edu_Tot_[MF]$",
                                      # "Tot_[MF]$",
                                      "Type_educanl_institution_ns_[MF]")] %>%
  drop_grep("Tert_Tot_Tert_[MF]") %>%
  melt.data.table(id.vars = Region_key,
                  value.name = "students") %>%

  .[!(variable %pin% "Type_educanl_institution_ns_[MF]"), EduInstitutionType := gsub("_[MF]$", "", variable)] %>%
  .[variable %pin% "Preschool",                          EduInstitutionType := "Pre-school"] %>%
  .[variable %pin% "Primary",                     EduInstitutionType := "Infants/Primary"] %>%
  .[variable %pin% "Secondary",                          EduInstitutionType := "Secondary"] %>%
  .[variable %pin% "Voc",                           EduInstitutionType := "Technical or Further Educational Institution"] %>%
  .[variable %pin% "Uni",                                EduInstitutionType := "University or other tertiary"] %>%
  .[variable %pin% "ns_.$",                                EduInstitutionType := NA_character_] %>%
  #
  .[, Sex := gsub("^.*([MF])$", "\\1", variable)] %>%
  .[] %>%
...
```{r Age_Volunteer_Sex, eval=FALSE}
# if (FALSE) {
freadG(23) %>%
  select_grep("^[MF]_", .and = Region_key) %>%
  drop_grep("Tot") %>%
  melt.data.table(id.vars = Region_key,
                  value.name = "adults") %>%
  .[, Age := extract_age(variable)] %>%
  .[, Sex := substr(variable, 0, 1)] %>%
  .[!grepl("ns$", variable), Volunteer := !grepl("N_a_", variable, ignore.case = TRUE)] %>%
  .[] %>%
  Mop("adults")
...
freadG(40) %>%
  ...
  .[grepl("_Ste?_ter", variable),        Landlord := "State or territory housing authority"] %>%
  .[grepl("Psn.not", variable),       Landlord := "Person not in same household"] %>%
  .[grepl("com_hou", variable),          Landlord := "Co-op/church group etc"] %>%
...
```{r G38}
freadG(41, na.strings = c("..")) %>%
  .[, .SD, .SDcols = ngrep("Tot", names(.))] %>%
  # select_grep("Dw(ellin)?gs", .and = Region_key) %>%
  drop_constant_cols() %>%
  melt.data.table(id.vars = Region_key,
                  value.name = "occupied_private_dwellings") %>%
  .[variable %pin% "Separate_house",        DwellingStructure :=  "Separate house"] %>%
  .[variable %pin% "SD_r_t_h",               DwellingStructure :=  "Semi-detached, row/terrace house, townhouse etc."] %>%
  ...
  .[variable %pin% "1_st?y?_",                DwellingSubtype :=    "1 storey"] %>%
  .[variable %pin% "1or2",                 DwellingSubtype :=    "1 or 2 storey block"] %>%
  .[variable %pin% "Se_d_r_or_t_h_t_2_s",  DwellingSubtype :=    "2 storeys or more"] %>%
  .[variable %pin% "3_st?y?_bl",               DwellingSubtype :=    "3 storey block"] %>%
  .[variable %pin% "Flt_apt_4to8",             DwellingSubtype :=    "4-8 storey block"] %>%
  .[variable %pin% "Flt_apt_9",             DwellingSubtype :=    "9 or more storey block"] %>%
  # .[variable == "Unoccupied_PDs_Dwgs", DwellingSubtype := "Unoccupied"] %>%
...
freadG(42) %>%
  ...
  .[variable %pin% "^FoA_4_to_8",     DwellingSubtype :=   "4-8 storey block"] %>%
  .[variable %pin% "^FoA_9mo",    DwellingSubtype :=   "9 or more storey block"] %>%
  #
...
if (Region == "SA1") {
  SA1_decoder <- unique(get(ced_tbls[1]), by = "SA1_CODE_2021")[[Region_key]]
  save(SA1_decoder,
       file = provide.file("~/Census2021.DataPack.SA1/data/SA1_intrnl_decoder.rda"))
}
...
if (Region == "SA1") {
  data_path <- provide_normalize("~/Census2021.DataPack.SA1/data")
  tsv_path <- provide_normalize(paste0("~/Census2021.DataPack.SA1/tsv/", Region))
} else {
  if (Region == "SSC") {
    data_path <- provide_normalize("~/Census2021.DataPack.SSC/data")
    tsv_path <- provide_normalize(paste0("~/Census2021.DataPack.SSC/tsv/", Region))
  } else {
    data_path <- "data"
...
CatR <- function(...) {
  cat(..., file = provide.file("./R/roxygen-data.R"), append = TRUE, sep = "")
  cat("\n", file = provide.file("./R/roxygen-data.R"), append = TRUE, sep = "")
}
if (Region == "STE") {
  CatR("#' @title Data pack data")
  CatR("#' @name Census2021__Data")
  CatR("#' @description Data from the 2021 Australian Census")
  CatR("#' NULL")
}

do_basic_roxygen <- function(x) {
  if (Region %in% c("SSC", "SA1")) {
    return(NULL)
  }
  CatR("#' @rdname ", x)
  CatR('"', x, '"')
...
lapply(ced_tbls, do_basic_roxygen)
...
fwrite <- function(...) data.table::fwrite(..., sep = "\t", logical01 = TRUE)
...
for (tbl in ced_tbls) {
  ...
  if (Region == "SA1") {
    .ced_tbl <- copy(.ced_tbl)
    .ced_tbl[, "SA1_internal_2021" := fastmatch::fmatch(SA1_CODE_2021, SA1_decoder)]
    .ced_tbl[, "SA1_CODE_2021" := NULL]
    set_cols_first(.ced_tbl, "SA1_internal_2021")
  }
