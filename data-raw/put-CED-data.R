...
UsualResidence_by_Age <-
  fread("./data-raw/data-packs/data/CED/AUST/2021Census_G03_AUS_CED.csv") %>%
  melt.data.table(id.vars = "CED_CODE_2021", variable.factor = FALSE) %>%
  .[!(variable %pin% "Tot")] %>%
  ...
  .[, .(CED_CODE_2021, Age, UsualResidence, value)] %>%
  setkey(CED_CODE_2021, Age, UsualResidence) %>%
  .[]

fread("./data-raw/data-packs/data/CED/AUST/2021Census_G01_AUS_CED.csv")
fread("./data-raw/data-packs/data/CED/AUST/2021Census_G02_AUS_CED.csv")
